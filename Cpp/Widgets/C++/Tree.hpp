...
class Tree {
	...
	private:
		...
		*/
		void rightBalance(){
			auto node = this->createRightVine(root);

			root = this->rightBalanceVine(node, this->getRightCount(node));
		}
		...
		*/
		void leftBalance(){
			auto node = this->createLeftVine(root);

			root = this->leftBalanceVine(node, this->getLeftCount(node));
		}
